# Memory engine — a curated LLM-Wiki

This is the shared "brain" every skill reads and writes. It makes generic skills **adapt
to an individual user over time**, using plain, human-readable, **version-controlled
markdown** — no agent-specific memory feature, no vector DB, no model training. The same
files work in GitHub Copilot, Claude Code, Cursor, Codex, and in **local scheduled
Workflows**.

## Design basis: the LLM-Wiki pattern (and why it's reliable)

We implement Andrej Karpathy's **LLM-Wiki** pattern
([karpathy/llm-wiki gist, 2026](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)).
Instead of RAG over raw logs ("rediscovering knowledge from scratch on every question"),
the agent maintains a **persistent, compounding** wiki it grooms itself. Three layers and
three operations:

| Layer | In Karpathy's pattern | Here |
|------|------------------------|------|
...
|-----------|--------------|-------------|
| **ingest** | record a decision/correction → update the relevant page(s) + `index.md` + append `log.md` | `comms-triage` (every run) |
| **query** | read `index.md` first, drill into the compiled pages, act | `comms-triage` (scoring) |
| **lint** | health checks: contradictions, stale claims, orphan/low-evidence entries, missing provenance, gaps → consolidate | `comms-tune` (the curator) |

Reliability comes from the same things that make a real wiki trustworthy and that the
pattern makes explicit: **provenance-first** (every learned claim cites its evidence),
**one concept per page** with cross-references, an **index** as the retrieval entry point,
an append-only **log**, **git history**, and a periodic **lint** pass that catches
contradictions and prunes stale claims. The human directs; the LLM does the bookkeeping.

This is well-trodden, credibly-sourced ground — see **Sources** at the bottom (Karpathy's
gist + "system prompt learning"; Reflexion; Generative Agents; MemGPT/Letta; the ACM
survey on agent memory; Anthropic's "structured note-taking"). It is *not* a random blog
pattern.
...
**Golden rule for unattended runs:** never block on input, never ask a question, and
**never send/post/merge anything unless `profile.md`'s `automation` block pre-authorizes
it.** Default to producing *drafts* + a *brief*; your later review is the confirmation.

---

## Where the wiki lives

Resolve `STATE_DIR` once per run: `$ENG_WORKFLOWS_STATE` if set, else `~/.eng-workflows/`.
Because Workflows run **locally on your machine**, this directory **persists between
runs** — no special handling needed. Keeping `STATE_DIR` under **git is recommended** (the
pattern's version-history + provenance), but it is *optional* and only for versioning/sync,
not for persistence.
...
~/.eng-workflows/
  ...
  log.md             # append-only: every triage run, decision, and lint pass (the raw layer)
  ...
  proposals.md       # lint/curation proposals awaiting your review
  wiki/
    people.md        # concept page: contacts that matter + handling (provenance + confidence)
...
If `STATE_DIR` has no `profile.md`: interactive → run onboarding (bottom); unattended →
deliver "profile not found, onboard once interactively" to the brief destination and exit.
...
- lint (comms-tune): flag contradictions, prune claims not confirmed in 30d unless conf=high,
...
- comms_tune_writes: propose-only       # propose-only | apply-safe | apply-all
```
...
### `cursors.md`
```markdown
- comms-triage: 2026-06-02T08:00Z
- comms-tune:   2026-05-26T07:30Z
```
...
Map every email / chat / channel post to one shape before scoring. Keep **snippets, not
full bodies** in any persisted artifact.
...
you're already in. Lower for `mute`/`low` source, `broadcast` you're not named in, `topics`
noise, big group threads you're only cc'd on. `rules.md` overrides. Always show a one-line
**why**.
...
- **Provenance over guesses.** A learned claim must cite evidence; if you can't, don't
  write it as a rule.
...
`profile.md`, `cursors.md`, and the `wiki/` pages from the seeds; ask 4–6 quick questions
(name/role/timezone, hours, tone, "up to 5 people who should always reach you"); set
`automation` conservatively (`draft-only`); optionally `git init`. Workflows assume this
ran once.
...
- Karpathy, **"system prompt learning"** — proposal that models improve by editing their own notes/instructions ([summary](https://x.com/koltregaskes/status/1921453031427649920), May 2025).
